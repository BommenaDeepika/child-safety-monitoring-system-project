...
def add_message_history_entry(alert_type, alert_message, delivery_status, sid=None, error_message=None):
    entry = {
        'id': f"{time.time_ns()}-{len(message_history)}",
    ...
    with message_history_lock:
        message_history.appendleft(entry)

    return entry


def update_message_history_status(message_sid, delivery_status, error_message=None):
    if not message_sid:
        return

    with message_history_lock:
        for entry in message_history:
            if entry.get('sid') == message_sid:
                entry['delivery_status'] = delivery_status
                entry['error_message'] = error_message
                return


def get_message_history_snapshot():
    with message_history_lock:
        return [dict(entry) for entry in message_history]
    ...
    try:
        ...
        def _log_delivery_status(message_sid):
            try:
                ...
                update_message_history_status(message_sid, 'status_unknown', str(status_error))
...
def esp_status_page():
    ...
    return render_template(
        ...
        message_history=get_message_history_snapshot()
    )
...
def get_esp_status():
    ...
    current_status['message_history'] = get_message_history_snapshot()
